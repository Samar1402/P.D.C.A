...
app.post("/addresult", (req, res) => {
  ...
  // Validation checks
  if (!first_team || typeof first_team !== "string" || first_team.trim() === "") {
    errors.first_team = "First team is required and must be a non-empty string.";
  }

  if (!second_team || typeof second_team !== "string" || second_team.trim() === "") {
    errors.second_team = "Second team is required and must be a non-empty string.";
  }
  ...
  if (!String(first_team_score) || first_team_score < 0) {
    errors.first_team_score = "First team score must be a non-negative integer.";
  }

  if (!String(second_team_score) || second_team_score < 0) {
    errors.second_team_score = "Second team score must be a non-negative integer.";
  }

  if (!winning_team || typeof winning_team !== "string" || winning_team.trim() === "") {
    errors.winning_team = "Winning team is required and must be a non-empty string.";
  }

  if (winning_team !== first_team && winning_team !== second_team) {
    errors.winning_team = "Winning team must be either the first team or the second team.";
  }

  if (
    winning_team_batsone &&
    (typeof winning_team_batsone !== "string" || winning_team_batsone.trim() === "")
  ) {
    errors.winning_team_batsone = "Winning team batsman one must be a valid string if provided.";
  }

  if (batsone_score !== undefined && (!String(batsone_score) || batsone_score < 0)) {
    errors.batsone_score = "Batsman one score must be a non-negative integer if provided.";
  }

  if (
    winning_team_batstwo &&
    (typeof winning_team_batstwo !== "string" || winning_team_batstwo.trim() === "")
  ) {
    errors.winning_team_batstwo = "Winning team batsman two must be a valid string if provided.";
  }

  if (batstwo_score !== undefined && (!String(batstwo_score) || batstwo_score < 0)) {
    errors.batstwo_score = "Batsman two score must be a non-negative integer if provided.";
  }

  if (
    winning_team_bowlerone &&
    (typeof winning_team_bowlerone !== "string" || winning_team_bowlerone.trim() === "")
  ) {
    errors.winning_team_bowlerone = "Winning team bowler one must be a valid string if provided.";
  }
  ...
  if (
    ...
    errors.bowlerone_wicket = "Bowler one wicket count must be a non-negative integer if provided.";
  }
...
app.post("/updateresult/:ID", (req, res) => {
  ...
  // Validate ID
  if (!ID || isNaN(ID)) {
    return res.status(400).json({ message: "Invalid or missing ID in the URL." });
  }
