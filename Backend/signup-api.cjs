...
//   if (contact && (!/^\d{1,10}$/.test(contact) || contact.length > 10)) {
//     errors.push("Contact No. must contain up to 10 digits only");
//   }
...
//   const query =
//     "INSERT INTO sign_up (first_name, last_name, email, contact, password) VALUES (?, ?, ?, ?, ?)";
//   req.db.query(
//     query,
//     [first_name, last_name, email, contact, password],
//     (err, result) => {
...
//   const { ID } = req.params;
//   const query = "DELETE FROM sign_up WHERE id = ?";
...
//   const query = `UPDATE sign_up SET ${fields.join(", ")} WHERE id = ?`;
//   values.push(ID);
